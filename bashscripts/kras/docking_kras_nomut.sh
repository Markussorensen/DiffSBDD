...
PROTEIN="kras"
PDB_ID_NEW="8AZV"
PDB_IDS_OTHER=("5VPI" "6OIM" "7OK3" "7RT1" "7TLG" "8AZZ" "8CX5" "8B00")
...
PDB_NEW_BINDING_POCKET="A:7 A:9 A:10 A:11 A:58 A:60 A:61 A:62 A:63 A:64 A:65 A:68 A:69 A:72 A:78 A:88 A:92 A:95 A:96 A:99 A:100 A:102 A:103"
MODEL="moad"
...
# Loop over the PDB IDs and queue the docking jobs, where they are docked against the new PDB.
# counter2=0
# for pdb_id_gen in "${PDB_IDS_OTHER[@]}"; do
#     SDF_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$pdb_id_gen/$MODEL/"
#     OUT_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$pdb_id_gen/$MODEL/sdf_out_$PDB_ID_NEW/"
#     # Use tsp to queue the job
#     tsp -N 1 python analysis/docking.py \
#             --pdbqt_dir $PDBQT_DIR \
#             --sdf_dir $SDF_DIR \
#             --out_dir $OUT_DIR \
#             --new_receptor_target $PDB_ID_NEW \
#             --new_target_binding_pocket $PDB_NEW_BINDING_POCKET \
#             --write_csv \
#             --write_dict
#     ((counter2++))
# done

# For the new PDB, dock against the other PDBs
counter=0
pdb_length=${#PDB_IDS_OTHER[@]}
while [ $counter -lt $pdb_length ]; do
    # Defining the dirs
    pdb_id_pdbfile=${PDB_IDS_OTHER[$counter]}
    binding_pocket=${PDB_BINDING_POCKETS[$counter]}
    SDF_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$PDB_ID_NEW/$MODEL/"
    OUT_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$PDB_ID_NEW/$MODEL/sdf_out_$pdb_id_pdbfile/"
    # Use tsp to queue the job
    mkdir -p $OUT_DIR
    tsp -N 1 python analysis/docking.py \
            --pdbqt_dir $PDBQT_DIR \
...
#Dock against the known inhibitors
SDF_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/inhibitors/"
OUT_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$PDB_ID_NEW/$MODEL/sdf_out_inhibitors/"
...
#Dock against own PDB as well
SDF_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$PDB_ID_NEW/$MODEL/"
OUT_DIR="/data/user/mas/DiffSBDD/data/cancer/$PROTEIN/results/$PDB_ID_NEW/$MODEL/sdf_out_$PDB_ID_NEW/"
#Make dir if it doesn't exist
mkdir -p $OUT_DIR
tsp -N 1 python analysis/docking.py \
        --pdbqt_dir $PDBQT_DIR \
